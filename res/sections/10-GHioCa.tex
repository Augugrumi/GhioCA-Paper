...
\begin{itemize}
  ...
  \item \texttt{CameraPreviewActivity} (Figure~\ref{fig:mainActivity}): this one displays the user a camera
    ...
    \end{figure}
  \item \texttt{ResultActivity}: these are two different activities depending on
        the type of image recognition chosen.
    \begin{itemize}
     \item If the ``reverse image search'' functionality is enabled then the
           result activity will contain a photo thumbnail (Figure~\ref{fig:imageResultActivity}), a description and a list of
...
open source and freely available on Github\footnote{\url{https://github.com/Augugrumi/CameraFragment}}. Because of some
